...
async function initDatabaseImageRegistry() {
    try {
        const res = await fetch(FIREBASE_IMAGE_REGISTRY_URL);
        if (res.ok) {
            const data = await res.json();
            if (data && typeof data === 'object') {
                applyDatabaseImageRegistry(data);
            }
        }
    } catch(e) {}

    // Live Real-Time SSE Stream for Instant Updates from Firebase Database
    try {
            ...
            evtSource.addEventListener('put', (e) => {
                try {
                    ...
                    if (payload && payload.data) {
                        applyDatabaseImageRegistry(payload.data);
                    }
                } catch(err) {}
            });
